...
def serialize_task(task: Any) -> dict:
    ...
    ``artifacts`` is included so the main agent knows where to find output
    files generated by the worker (backtest results, reports, etc.).
    Artifact paths are redacted to scrub any internal path prefixes that
    may have been stored (defence in depth — workers normally store relative
    paths, but an absolute path must not leak the host topology).
    """
    status = task.status.value if hasattr(task.status, "value") else str(task.status)
    raw_artifacts = list(getattr(task, "artifacts", []) or [])
    redacted_artifacts = [redact_internal_paths(p) for p in raw_artifacts]
    return {
        "id": task.id,
