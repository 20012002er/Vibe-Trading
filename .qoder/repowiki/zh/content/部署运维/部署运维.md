...
10. [安全加固与防护策略](#安全加固与防护策略)
11. [CI/CD 集成](#cicd-集成)
12. [故障排查指南](#故障排查指南)
13. [结论](#结论)

## 简介
本文件面向 Vibe-Trading 的容器化部署与运维，覆盖镜像构建（多阶段优化）、环境变量配置、健康检查、生产与负载均衡、监控告警、日志收集、备份恢复、CI/CD 集成以及常见问题处理。文档基于仓库中的 Dockerfile、docker-compose、API 服务入口与环境配置模型等实际代码进行说明，并提供可操作的部署示例与最佳实践。**最新更新**：增加了生产环境安全加固指导，包括shell工具启用程序、网络传输安全配置和DNS重绑定攻击缓解策略。
...
- **新增安全特性**：Host/Origin 白名单，防止 DNS 重绑定攻击；shell 工具默认关闭，需显式开启。
...
### Shell工具启用程序
**重要安全特性**：Shell-capable 工具（bash、background_run、cancel_background）在所有传输模式下默认禁用，需要显式启用。
...
2. **命令行参数方式**：
   ```bash
   python mcp_server.py --enable-shell-tools
   ```

#### 安全考虑
- 这些工具是远程过程控制表面，一旦可达任何客户端都存在安全风险
- 即使通过 stdio 传输也保持禁用状态，除非显式启用
- 禁止按进程名终止 Python 进程，避免意外终止 Vibe-Trading 自身
...
### 网络传输安全配置
**新增**：MCP 服务内置 DNS 重绑定攻击防护机制。

#### 主机白名单机制
- **默认配置**：仅允许 localhost、127.0.0.1、::1
- **自定义配置**：通过 `VIBE_TRADING_MCP_ALLOWED_HOSTS` 环境变量设置
- **支持格式**：
  - 精确匹配：`example.com`
  - 子域名匹配：`*.example.com`
  - 通配符：`*`（允许所有主机）
...
```bash
# 允许特定域名
export VIBE_TRADING_MCP_ALLOWED_HOSTS="api.example.com,*.example.com,127.0.0.1"

# 允许所有主机（不推荐生产环境）
export VIBE_TRADING_MCP_ALLOWED_HOSTS="*"
```

**章节来源**
- [agent/mcp_server.py:130-192](file://agent/mcp_server.py#L130-L192)
- [agent/mcp_server.py:208-227](file://agent/mcp_server.py#L208-L227)

### DNS重绑定攻击缓解策略
**新增**：针对浏览器驱动的 DNS 重绑定攻击的全面防护。

#### 攻击原理
DNS 重绑定攻击利用浏览器同源策略的限制，通过恶意网页将域名解析到内部地址，从而绕过安全限制访问本地服务。

#### 多层防护机制
1. **Host头验证**：验证请求的目标主机是否在白名单中
2. **Origin头验证**：验证请求来源是否可信
3. **默认安全策略**：仅允许本地回环地址
4. **IPv6支持**：正确处理 IPv6 地址格式

#### 防护措施实现
- `_HostGuardMiddleware`：验证 Host 头
- `_OriginGuardMiddleware`：验证 Origin 头  
- `_normalize_host`：标准化主机名比较
- `_origin_allowed`：来源域名验证

**章节来源**
- [agent/mcp_server.py:130-140](file://agent/mcp_server.py#L130-L140)
- [agent/mcp_server.py:229-286](file://agent/mcp_server.py#L229-L286)
...
- **新增**：DNS重绑定攻击相关故障
  - 检查 `VIBE_TRADING_MCP_ALLOWED_HOSTS` 配置是否正确
  - 确认请求的 Host 和 Origin 头都在白名单中
  - 查看服务器日志中的 "Invalid host header" 或 "Origin not allowed" 错误
...
- [Dockerfile:62-75](file://Dockerfile#L62-L75)
- [agent/mcp_server.py:93-127](file://agent/mcp_server.py#L93-L127)
- [agent/mcp_server.py:250-256](file://agent/mcp_server.py#L250-L256)
- [agent/mcp_server.py:279-285](file://agent/mcp_server.py#L279-L285)

## 结论
Vibe-Trading 提供了完善的容器化与生产就绪能力：多阶段构建最小化镜像、严格的安全加固、集中化的环境变量与配置体系、健壮的健康检查与日志脱敏、以及可扩展的 MCP 工具生态。**最新更新**：增强了生产环境安全加固指导，包括shell工具启用程序、网络传输安全配置和DNS重绑定攻击缓解策略。结合反向代理、负载均衡、监控告警与备份恢复策略，可在生产环境中稳定运行并持续演进。