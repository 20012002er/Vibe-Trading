...
- 辅助模块：符号类型识别（_symbol_utils）、A股复权（cn_adjust）等。
...
- 重试与预算：retry_with_budget/check_budget 以 wall-clock 预算和指数退避保护外部 API。
...
- 代理与超时：CCXT/OKX 支持 ALL_PROXY/HTTP_PROXY/HTTPS_PROXY；可通过环境变量调整超时与预算。
