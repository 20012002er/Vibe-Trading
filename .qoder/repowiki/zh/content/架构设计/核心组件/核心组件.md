...
## 更新摘要
**变更内容**
- 增强了身份与证据门控系统（GroundingLedger）的竞态条件保护机制
- 改进了对裸美国股票代码的处理逻辑，特别针对 SEC 相关工具
- 增强了对陈旧历史身份的防护措施
- 优化了部分解析器故障时的错误处理流程
- 完善了 get_market_data 工具对解析结果的消费保护机制
...
- **更新** 增强的竞态条件保护机制
  - 通过批次 ID 跟踪机制防止 search_symbol 和 get_market_data 在同一批次中的竞态问题
  - 实现了批次边界检测，确保解析结果在下一批次前不可被消费
  - 增强了历史身份隔离，防止陈旧身份解锁新主体的工具调用
- **更新** 改进的裸美国股票代码处理
  - 为 SEC 相关工具（如 get_sec_filings、trading_quote 等）专门处理裸股票代码
  - 允许 AAPL 匹配 AAPL.US，但禁止静默后缀重写（如 SH/SS）
  - 严格区分已锁定身份和临时符号
- **更新** 增强的错误处理
  - 改进了部分解析器故障时的处理逻辑
  - 当单一来源不可用时，不会错误地将公开实体路由到私人研究
  - 增强了身份冲突检测和验证流程
- 关键流程
...
participant GL as "GroundingLedger"
participant TR as "ToolRegistry"
AL->>GL : authorize_tool_call(tool, args, batch_symbols, call_id, batch_id)
alt 允许
AL->>TR : 执行工具
TR-->>AL : 结果(JSON)
...
Note over GL : 批次ID跟踪防止竞态条件
Note over GL : 裸股票代码特殊处理
Note over GL : 陈旧身份防护
```

**图表来源**
- [grounding.py:666-753](file://agent/src/agent/grounding.py#L666-L753)
- [grounding.py:789-800](file://agent/src/agent/grounding.py#L789-800)
...
- **更新** 身份门控拦截
  ...
  - **新增** 竞态条件保护：如果在同一批次中同时调用 search_symbol 和 get_market_data，后者会被阻止直到下一批次。
  - **新增** 裸股票代码处理：对于 SEC 相关工具，可以使用裸股票代码（如 AAPL），但会匹配对应的完整符号（AAPL.US）。
  - **新增** 陈旧身份防护：前一轮的 AAPL 身份不能解锁新的 SpaceX 价格请求。
...
**更新** 最新的改进进一步增强了系统的健壮性：
- 通过竞态条件保护机制防止了解析器和消费者工具之间的时序问题
- 改进了对裸美国股票代码的处理，特别针对 SEC 相关工具
- 增强了对陈旧历史身份的防护，避免身份泄露
- 优化了部分解析器故障时的错误处理流程
...
- 场景一：快速市场数据查询
  ...
  - **更新** 注意竞态条件：不要在同一个批次中同时调用 search_symbol 和 get_market_data。
- 场景二：策略回测与分析
...
- **更新** 最佳实践
  ...
  - 对于 SEC 相关工具，可以使用裸股票代码，但系统会自动匹配完整符号。
  - 避免依赖陈旧的历史身份信息，确保每次会话都重新解析身份。
