...
## 更新摘要
**变更内容**
- 更新了工具注册系统的安全默认配置，现在默认禁用shell工具
- 添加了显式启用shell工具的机制和环境变量配置
- 增强了安全沙箱和命令安全检查功能
- 更新了部署指南和安全最佳实践
...
**重要安全更新**：从版本开始，工具注册系统采用"失败关闭"的安全默认策略，默认禁用所有shell工具（bash、background_run、cancel_background），必须通过显式配置才能启用。这一变更显著提升了系统的安全性，防止潜在的远程代码执行风险。
...
F --> H["Shell安全检查<br/>agent/src/tools/_shell_safety.py"]
```
...
Reg->>Reg : 检查 include_shell_tools 配置
alt 默认配置 (False)
Reg->>Reg : 跳过 bash/background_run/cancel_background
else 显式启用 (True)
Reg->>Reg : 包含所有工具
end
Reg->>R : 注册本地工具含可选 MCP
...
- [agent/src/agent/loop.py:624-800](file://agent/src/agent/loop.py#L624-800)
- [agent/src/tools/bash_tool.py:16-84](file://agent/src/tools/bash_tool.py#L16-L84)
...
**更新**：新增了严格的安全策略，默认禁用所有shell工具，需要显式配置才能启用。
...
Filter -- 否 (默认) --> SkipShell["跳过 bash/background_run/cancel_background"]
Filter -- 是 (显式启用) --> KeepAll["保留全部"]
...
- 使用示例（概念性）：
  ...
  - 脚本执行：**仅在显式启用shell工具的环境下**，在允许环境下启用 bash_tool，在 run_dir 内执行安装/脚本任务。

**更新**：强调了shell工具需要在显式启用的环境下才能使用。
...
Config -.安全配置.-> Security["Shell工具安全策略"]
```
...
- **Shell 工具不可用**：确认是否已启用 include_shell_tools；检查命令安全检查是否拦截；设置 `VIBE_TRADING_ENABLE_SHELL_TOOLS=1` 环境变量。
...
## 结论
Vibe-Trading 的工具注册系统通过清晰的抽象、自动发现与动态加载、严格的沙箱与超时控制，以及可扩展的 MCP 集成，提供了安全、高效、可观测的工具执行基础设施。**最新的安全增强措施确保了shell工具默认禁用，需要显式配置才能启用，大大提升了系统的整体安全性**。遵循本文的接口规范与实践建议，可快速扩展高质量工具并融入现有工作流。
...
- **安全测试**：验证默认不暴露 shell 工具，仅在显式开启时注册；测试环境变量和CLI标志的正确行为。
...
- **生产环境**：
  ...
  - **如需启用shell工具**：设置 `VIBE_TRADING_ENABLE_SHELL_TOOLS=1` 环境变量或使用 `--enable-shell-tools` CLI标志。

**更新**：新增了生产环境的shell工具启用指导。
...
### 安全配置最佳实践
- **默认安全策略**：所有远程API和MCP服务器默认禁用shell工具，这是"失败关闭"的安全设计。
- **显式启用**：仅在完全信任的环境中启用shell工具，并通过环境变量或CLI标志明确配置。
- **命令安全检查**：系统会自动阻止危险的Python进程终止命令，防止意外终止Vibe-Trading自身。
- **输出限制**：shell工具的输出会被限制在50KB以内，防止内存溢出攻击。
- **超时保护**：所有shell命令都有120秒的超时限制，避免长时间运行的恶意脚本。
