...
- [agent/backtest/loaders/registry.py:1-252](file://agent/backtest/loaders/registry.py#L1-L252)
...
- [agent/src/tools/pattern_tool.py:323-411](file://agent/src/tools/pattern_tool.py#L323-L411)
- [agent/src/tools/orderbook_depth_tool.py:285-445](file://agent/src/tools/orderbook_depth_tool.py#L285-L445)
- [agent/backtest/loaders/registry.py:136-252](file://agent/backtest/loaders/registry.py#L136-L252)
...
图表来源
- [agent/src/market_data.py:97-223](file://agent/src/market_data.py#L97-L223)
- [agent/backtest/loaders/registry.py:199-252](file://agent/backtest/loaders/registry.py#L199-L252)
...
- [agent/backtest/loaders/registry.py:221-252](file://agent/backtest/loaders/registry.py#L221-L252)
...
**更新** 数据源优先级策略已优化，特别针对中国大陆用户体验进行了改进

- **首选源推断**：依据符号后缀/格式（A 股、美股、港股、印度、加拿大、期货/外汇、韩股、加密货币）
- **回退链策略**：按市场类型定义优先顺序，重点关注以下改进：
  - **美股数据**：`us_equity` 链现在优先使用 `yahoo` 公共端点，然后是国内数据源（eastmoney/akshare/sina），最后才是 yfinance SDK
  - **港股数据**：`hk_equity` 链优先使用 `tencent`（永不封禁），然后是 akshare（东方财富支持），再是 yahoo，最后是 yfinance
  - **A股数据**：`a_share` 链保持最优顺序：tencent → mootdx → eastmoney → baostock → akshare → tushare → local
- **特殊约束**：local/qveris 明确禁止静默回退到网络源，避免掩盖配置问题
- **中国大陆优化**：通过优先使用国内数据源（tencent/eastmoney/akshare/sina），有效避免了 Yahoo 服务在中国大陆的限速问题

**参考路径**：[market_data.py:16-57](file://agent/src/market_data.py#L16-L57), [registry.py:136-155](file://agent/backtest/loaders/registry.py#L136-L155), [registry.py:221-252](file://agent/backtest/loaders/registry.py#L221-L252)
...
**更新** 以下是各市场的详细数据源优先级策略：

#### A股市场 (`a_share`)
1. `tencent` - 腾讯财经（永不封禁）
2. `mootdx` - 通达信TCP（永不封禁）
3. `eastmoney` - 东方财富（可能限速）
4. `baostock` - 免费A股数据
5. `akshare` - 开源金融数据
6. `tushare` - 需要token
7. `local` - 本地数据
...
#### 美股市场 (`us_equity`)
1. `yahoo` - Yahoo公共端点（无SDK限速）
2. `eastmoney` - 东方财富（国内数据源）
3. `akshare` - 开源金融数据
4. `sina` - 新浪财经
5. `stooq` - 全球EOD数据
6. `tiingo` - 需要API key
7. `fmp` - Financial Modeling Prep
8. `finnhub` - 需要API key
9. `alphavantage` - 需要API key
10. `longbridge` - 长桥证券
11. `local` - 本地数据
12. `yfinance` - Yahoo Python SDK（最后备选）
...
9. `longbridge` - 长桥证券
10. `local` - 本地数据
11. `yfinance` - Yahoo Python SDK（最后备选）

这种优先级策略确保了在中国大陆地区能够获得最佳的数据访问体验，同时保持了系统的健壮性和可靠性。

**参考路径**：[registry.py:136-155](file://agent/backtest/loaders/registry.py#L136-L155)