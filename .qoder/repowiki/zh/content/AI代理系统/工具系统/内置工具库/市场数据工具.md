...
## 更新摘要
**所做更改**
- 更新了 SymbolSearchTool 章节，详细说明新增的中国查询检测机制 (_is_china_query 函数)
- 增强了数据源回退链说明，突出东方财富优先于雅虎财经的策略
- 完善了错误处理部分，详细说明每源状态跟踪机制
- 更新了架构图和流程图以反映新的智能源选择逻辑
...
## 架构总览
统一的数据获取流程由 fetch_market_data_json 驱动，内部根据符号格式推断首选数据源，并按市场维度的回退链尝试各加载器，直至成功或耗尽尝试次数。**SymbolSearchTool 现已集成智能查询检测机制，对于中国市场的查询（包含中文字符或纯数字 A 股代码），自动跳过 Yahoo Finance 以避免中国大陆 IP 的限制，优先使用东方财富数据源。**
...
## 结论
Vibe-Trading 的市场数据工具集通过统一接口屏蔽了多市场、多资产的复杂性，借助符号检测与市场级回退链实现高可用数据获取；**SymbolSearchTool 现已集成智能中国查询检测机制，显著提升了中国大陆用户的搜索体验，避免了 Yahoo Finance 的限制问题**；sector_tool 聚焦 A 股板块洞察。三者共同构成研究、分析与交易工作流的基础设施。建议在生产环境中结合缓存与监控，进一步提升稳定性与性能。
