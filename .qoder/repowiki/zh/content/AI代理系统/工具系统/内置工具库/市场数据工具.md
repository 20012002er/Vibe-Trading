...
## 更新摘要
**所做更改**
- 新增接地系统集成章节，详细说明 get_market_data 工具的竞态条件保护和身份验证豁免机制
- 增强安全边界处理章节，说明从多种来源（筛选器、研究结果）处理符号的安全机制
- 更新架构总览图以反映新的接地系统集成和批量处理保护
- 完善错误处理部分，增加竞态条件检测和身份冲突处理
- 新增调用示例，展示与 resolver 调用的正确交互模式
...
4. [架构总览](#架构总览)
5. [接地系统集成](#接地系统集成)
6. [HTTP客户端基础设施](#http客户端基础设施)
7. [详细组件分析](#详细组件分析)
8. [依赖关系分析](#依赖关系分析)
9. [性能与缓存](#性能与缓存)
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)
12. [附录：调用示例与数据处理模式](#附录调用示例与数据处理模式)

## 简介
本文件系统性介绍 Vibe-Trading 的市场数据工具集，重点覆盖以下四个工具：
- market_data_tool：统一的多市场、多资产实时与历史 OHLCV 数据获取入口，支持自动源选择与按市场回退链。**现已增强接地系统集成，提供更好的竞态条件保护和从多种来源处理符号的安全机制。**
...
## 项目结构
围绕市场数据工具的核心代码分布在 agent/src/tools 与 agent/src 的共享模块中，并通过 backtest.loaders.registry 提供加载器注册与回退链。测试用例位于 agent/tests，用于验证行为与边界条件。HTTP客户端基础设施位于 agent/backtest/loaders/_http.py，为所有数据源提供统一的限流和连接管理。**新增的接地系统集成位于 agent/src/agent/grounding.py，负责工具调用的身份验证和安全边界控制。**
...
subgraph "接地系统"
GL["GroundingLedger<br/>身份验证+竞态保护"]
RS["Resolver Race Guard<br/>批量处理保护"]
EX["Exemption Logic<br/>只读工具豁免"]
end
subgraph "共享能力"
...
- [agent/src/tools/market_screener_tool.py:242-283](file://agent/src/tools/market_screener_tool.py:242-L283)
- [agent/src/agent/grounding.py:721-757](file://agent/src/agent/grounding.py#L721-L757)
...
- MarketDataTool（get_market_data）
  ...
  - **新增**：接地系统集成提供竞态条件保护和身份验证豁免，允许从多种来源安全地处理符号。
...
## 架构总览
统一的数据获取流程由 fetch_market_data_json 驱动，内部根据符号格式推断首选数据源，并按市场维度的回退链尝试各加载器，直至成功或耗尽尝试次数。**SymbolSearchTool 现已集成智能查询检测机制，对于中国市场的查询（包含中文字符或纯数字 A 股代码），自动跳过 Yahoo Finance 以避免中国大陆 IP 的限制，优先使用东方财富数据源。** 所有HTTP请求都通过统一的HTTP客户端基础设施进行限流和连接管理。**新增的接地系统集成确保 get_market_data 工具在批量处理中与 resolver 调用保持正确的时序关系，防止竞态条件。**

```mermaid
sequenceDiagram
participant U as "调用方"
participant G as "GroundingLedger<br/>身份验证"
...
### 竞态条件保护
接地系统为 get_market_data 工具提供了强大的竞态条件保护机制，确保在与 resolver 调用并发执行时的安全性：

- **批量处理保护**：跟踪当前批次的 resolver 调用ID，防止在同一批次中消费尚未完成的 resolver 结果
- **批次边界检测**：通过单调递增的 batch_id 检测批次边界，重置 resolver 跟踪器
- **时序保证**：确保 resolver 结果在下一批次之前不会被消费，避免数据不一致

### 身份验证豁免机制
get_market_data 作为只读数据工具，享有特殊的身份验证豁免：

- **只读工具特性**：不执行任何写操作，仅读取市场数据，因此不需要严格的身份锁定
- **多源符号支持**：允许从各种来源（筛选器、研究结果等）获取符号，即使与解析器的锁定身份不同
- **合法使用场景**：支持筛选器符号与 search_symbol 结果后缀不同的合法使用场景

### 安全边界控制
接地系统在允许灵活性的同时保持严格的安全边界：

- **终端状态检查**：当身份处于模糊、冲突或无效状态时，阻止 get_market_data 调用
- **批量上下文隔离**：每个批次的 resolver 调用结果不会污染其他批次的授权状态
- **错误处理**：提供清晰的错误消息，指导用户正确的调用顺序

```mermaid
flowchart TD
Start["工具调用开始"] --> CheckType{"工具类型?"}
CheckType --> |get_market_data| IdentityReq{"需要身份验证?"}
CheckType --> |其他工具| StandardAuth["标准身份验证流程"]
IdentityReq --> |是| BatchCheck{"检查批量resolver调用"}
BatchCheck --> |有未完成的resolver| Block["阻止调用<br/>等待下一批次"]
BatchCheck --> |无未完成resolver| StatusCheck{"检查身份状态"}
StatusCheck --> |终端坏状态| Block
StatusCheck --> |正常状态| Allow["允许调用<br/>只读工具豁免"]
IdentityReq --> |否| Allow
StandardAuth --> Result["返回授权结果"]
Block --> Result
Allow --> Result
```
...
**图表来源**
- [agent/src/agent/grounding.py:721-757](file://agent/src/agent/grounding.py#L721-L757)
- [agent/src/agent/grounding.py:672-700](file://agent/src/agent/grounding.py#L672-L700)

**章节来源**
- [agent/src/agent/grounding.py:721-757](file://agent/src/agent/grounding.py#L721-L757)
- [agent/src/agent/grounding.py:672-700](file://agent/src/agent/grounding.py#L672-L700)
- [agent/tests/test_agent_grounding.py:430-465](file://agent/tests/test_agent_grounding.py#L430-L465)
...
**章节来源**
- [agent/backtest/loaders/_http.py:1-224](file://agent/backtest/loaders/_http.py#L1-L224)
...
- 关键实现要点
  ...
  - **接地集成**：通过 GroundingLedger 进行身份验证，支持批量处理中的竞态条件保护。
- 错误处理
  - 单个加载器异常不会中断整体流程，会记录日志并继续尝试下一个回退源。
  - 无法解析的符号汇总到 _unresolved 字段。
  - **接地错误**：当存在未完成的 resolver 调用或身份状态异常时，返回明确的错误消息。
- 性能特性
  - 行裁剪 cap_rows 控制响应大小，默认上限可配置。
  - 批量 codes 同组处理，减少重复逻辑。
  - **批量优化**：接地系统确保批量处理中的正确时序，避免不必要的重试。

```mermaid
flowchart TD
Start(["开始"]) --> Auth["接地系统验证<br/>竞态条件检查"]
Auth --> |允许| Detect["检测符号首选源"]
Auth --> |拒绝| Error["返回身份错误"]
...
- [agent/tests/test_market_screener_tool.py:43-165](file://agent/tests/test_market_screener_tool.py:43-L165)

**章节来源**
- [agent/src/tools/market_screener_tool.py:1-340](file://agent/src/tools/market_screener_tool.py#L1-L340)
- [agent/tests/test_market_screener_tool.py:1-165](file://agent/tests/test_market_screener_tool.py:1-L165)
...
- 市场数据获取
  ...
  - **接地集成**：通过 GroundingLedger 进行身份验证和竞态条件保护。
- 符号搜索
...
GL["grounding.GroundingLedger<br/>竞态保护+身份验证"] --> MDT["MarketDataTool"]
MDT --> MD
...
- get_market_data
  ...
  - **新增**：如果收到 identity_required 错误，检查是否在同一个批次中调用了 search_symbol，需要等待下一批次。
- search_symbol
  - 现象：某源失败但仍有结果。
...
- [agent/src/tools/market_screener_tool.py:285-340](file://agent/src/tools/market_screener_tool.py#L285-L340)
- [agent/src/agent/grounding.py:731-740](file://agent/src/agent/grounding.py#L731-L740)

## 结论
Vibe-Trading 的市场数据工具集通过统一接口屏蔽了多市场、多资产的复杂性，借助符号检测与市场级回退链实现高可用数据获取；**SymbolSearchTool 现已集成智能中国查询检测机制，显著提升了中国大陆用户的搜索体验，避免了 Yahoo Finance 的限制问题**；sector_tool 聚焦 A 股板块洞察，**现已支持多源回退提高可靠性**；**新增的 market_screener_tool 提供多市场筛选能力，支持A股、美股、港股三大市场的股票筛选**。三者共同构成研究、分析与交易工作流的基础设施。**新增的HTTP客户端基础设施为所有数据源提供统一的限流和连接管理，显著提升系统稳定性和性能**。**最重要的是，接地系统集成为 get_market_data 工具提供了强大的竞态条件保护和身份验证豁免机制，确保在与 resolver 调用并发执行时的安全性和正确性，同时允许从多种来源（筛选器、研究结果等）安全地处理符号**。建议在生产环境中结合缓存与监控，进一步提升稳定性与性能。
...
- **接地系统集成示例**
  - 正确模式：先调用 search_symbol 解析符号，等待结果后再调用 get_market_data
  - 错误模式：在同一批次中同时调用 search_symbol 和 get_market_data，会导致竞态条件错误
  - 豁免特性：get_market_data 可以从筛选器或其他来源获取符号，不受身份锁定限制
  - 参考：[agent/tests/test_agent_grounding.py:430-465](file://agent/tests/test_agent_grounding.py#L430-L465)
...
- 数据处理模式建议
  ...
  - **接地系统最佳实践**：避免在同一批次中混合调用 resolver 和数据获取工具，确保正确的执行时序。
