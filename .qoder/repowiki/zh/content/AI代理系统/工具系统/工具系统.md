...
## 架构总览
下图展示从 Agent 侧发起工具调用到具体工具执行的端到端流程，包括注册、发现、参数校验、执行与安全扫描。
...
LLM->>Reg : execute("get_market_data", {codes,start_date,end_date,...})
...
## 结论
Vibe-Trading 工具系统通过统一的 BaseTool 接口与 ToolRegistry 注册表，结合自动发现与动态加载，实现了高扩展性与易维护性。内置工具覆盖回测、市场数据与因子分析等核心场景，并通过安全扫描与沙箱执行保障稳定性与安全性。开发者可按规范快速扩展自定义工具，借助参数 schema 与标准返回格式与 Agent 无缝集成。
