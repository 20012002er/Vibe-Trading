...
## 更新摘要
**变更内容**
- 更新了美股和港股数据源回退链优先级，优化了中国大陆地区的网络访问稳定性
- 将国内数据源（东方财富、AKShare、新浪财经）置于 yfinance 之前，解决速率限制和IP封禁问题
- 增强了回退链的容错性和可用性保障机制
...
**更新** 针对中国大陆地区访问优化，调整了美股和港股的回退链优先级，优先使用国内数据源以提高稳定性和可用性。
...
## 结论
Vibe-Trading 的数据源集成通过"协议 + 注册表 + 回退链"实现了高内聚、低耦合的统一接入。各数据源遵循相同接口，屏蔽差异化的时间戳、列名与认证方式；回退链在保证可用性的同时，兼顾了网络稳定性与数据质量。**最新更新** 针对中国大陆地区访问进行了优化，将国内数据源置于 yfinance 之前，有效解决了速率限制和 IP 封禁问题。结合重试预算与本地缓存，系统在大规模回测与实时场景中具备鲁棒性与高性能。
