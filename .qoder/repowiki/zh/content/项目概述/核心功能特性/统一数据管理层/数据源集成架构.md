...
- 注册表与回退链
  ...
  - FALLBACK_CHAINS：按市场类型定义优先顺序，兼顾 IP 封禁风险与数据质量。
...
- 市场映射：FALLBACK_CHAINS 为每个市场定义有序候选源，例如：
...
图表来源
- [akshare_loader.py:74-157](file://agent/backtest/loaders/akshare_loader.py#L74-L157)
- [akshare_loader.py:158-292](file://agent/backtest/loaders/akshare_loader.py#L158-L292)

章节来源
- [akshare_loader.py:1-292](file://agent/backtest/loaders/akshare_loader.py#L1-L292)
...
章节来源
- [registry.py:158-193](file://agent/backtest/loaders/registry.py#L158-L193)
- [registry.py:221-249](file://agent/backtest/loaders/registry.py#L221-L249)
...
- A 股
  ...
  - 特点：多源冗余，公共源优先，本地兜底。
- 美股
  - 推荐链：yahoo → stooq → sina → eastmoney → yfinance → tiingo → fmp → finnhub → alphavantage → longbridge → akshare → local
  - 特点：免费公共源在前，付费 REST 在后。
- 港股
  - 推荐链：tencent → eastmoney → yahoo → futu → akshare → yfinance → tushare → longbridge → local
  - 特点：考虑大陆 IP 限制，Yahoo SDK 可能受限。
- 加密货币
...
### 配置与认证要点
- 无需认证
  - Yahoo、AKShare、CCXT（公开行情）、本地加载器（本地文件）。
- 需要认证
...
### 实际使用示例（描述性）
- 查询美股日线
  - 调用 resolve_loader("us_equity") 获取可用加载器（如 yahoo），再 fetch(["AAPL.US"], "2024-01-01", "2024-01-31", interval="1D")。
- 查询加密货币永续合约
