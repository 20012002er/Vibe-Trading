...
- [agent/backtest/loaders/stooq_loader.py](file://agent/backtest/loaders/stooq_loader.py)
...
## 更新摘要
**所做更改**
- 更新了网络控制与限流部分，新增HostThrottle类、进程级会话复用、指数退避重试逻辑和User-Agent旋转机制
- 增强了HTTP请求管理的安全性和稳定性
- 添加了金融数据提供商访问的健壮性保障
- 更新了故障排查指南以包含新的限流和重试机制
...
**最新更新**：增强了HTTP限流和连接管理系统，包括HostThrottle类、进程级会话复用、指数退避重试逻辑和自动User-Agent旋转机制，为金融数据提供商访问提供更强健的保障。
...
NET --> THROTTLE["HostThrottle<br/>进程级限流"]
NET --> SESSIONS["进程级会话池<br/>连接复用"]
...
- [agent/backtest/loaders/_http.py:98-110](file://agent/backtest/loaders/_http.py#L98-L110)
...
- [agent/src/core/runner.py:431-590](file://agent/src/core/runner.py#L431-L590)
- [agent/src/live/enforcement.py:455-617](file://agent/src/live/enforcement.py#L455-L617)
- [agent/backtest/loaders/_http.py:53-110](file://agent/backtest/loaders/_http.py#L53-L110)
- [frontend/src/lib/apiAuth.ts:1-21](file://frontend/src/lib/apiAuth.ts#L1-L21)
...
- **增强的网络控制**：按主机限速、进程级连接池复用、指数退避重试、自动User-Agent旋转、代理与证书透传。
...
- [agent/backtest/loaders/_http.py:53-196](file://agent/backtest/loaders/_http.py#L53-L196)
...
participant HTTP as "HTTP限流<br/>_http.py"
...
Runner->>HTTP : "throttled_get() 带限流"
HTTP->>HTTP : "HostThrottle.wait()"
HTTP->>HTTP : "指数退避重试"
HTTP-->>Runner : "带User-Agent的请求"
...
### 增强的网络控制与文件访问控制
**更新**：新增了增强的HTTP限流和连接管理系统，提供更健壮的金融数据提供商访问能力。

- **HostThrottle类**：进程级最小间隔门控，基于任意主机桶键控，确保同一主机的请求间隔符合最小间隔要求。
- **进程级会话复用**：按host bucket复用requests.Session，减少TCP/TLS握手开销，提高连接效率。
- **指数退避重试**：对瞬态网络错误（连接重置、超时、协议错误）实施指数退避重试，增强网络鲁棒性。
- **自动User-Agent旋转**：默认使用桌面浏览器User-Agent，支持自定义UA字符串，满足金融数据提供商的访问要求。
- **随机抖动**：在最小间隔基础上添加随机抖动，避免并发调用者同步触发限流。
- **代理与证书透传**：透传HTTP_PROXY/HTTPS_PROXY/ALL_PROXY/NO_PROXY与证书相关环境变量，确保数据源可达。
- **文件访问控制**：沙箱HOME仅暴露必要路径；生成代码无法读取真实~/.vibe-trading中的敏感文件与会话数据库。

```mermaid
flowchart TD
Request["HTTP请求"] --> Throttle["HostThrottle.wait()"]
Throttle --> Jitter["添加随机抖动"]
Jitter --> Session["获取进程级Session"]
Session --> Retry["指数退避重试循环"]
Retry --> Success{"请求成功?"}
Success --> |是| Return["返回响应"]
Success --> |否| Backoff["计算退避时间"]
Backoff --> ReWait["重新等待限流间隔"]
ReWait --> Retry
Return --> Close["最后重试时关闭旧连接"]
Close --> NewSession["创建新Session"]
NewSession --> Retry
```
...
**章节来源**
- [agent/backtest/loaders/_http.py:53-196](file://agent/backtest/loaders/_http.py#L53-L196)
- [agent/backtest/loaders/sec_edgar_client.py:53-96](file://agent/backtest/loaders/sec_edgar_client.py#L53-L96)
- [agent/backtest/loaders/sina_loader.py:191-197](file://agent/backtest/loaders/sina_loader.py#L191-L197)
- [agent/backtest/loaders/stooq_loader.py:155-160](file://agent/backtest/loaders/stooq_loader.py#L155-L160)
- [agent/src/core/runner.py:175-278](file://agent/src/core/runner.py#L175-L278)
- [agent/src/core/runner.py:113-172](file://agent/src/core/runner.py#L113-L172)
...
**图表来源**
- [agent/src/live/enforcement.py:455-617](file://agent/src/live/enforcement.py#L455-617)
- [agent/src/live/enforcement.py:620-676](file://agent/src/live/enforcement.py#L620-676)
...
- **增强的网络限流模块**被各数据加载器复用，保障稳定性与合规性，包括HostThrottle、进程级会话管理和指数退避重试。
...
HTTP["_http.py"] --> THROTTLE["HostThrottle"]
HTTP --> SESSIONS["进程级会话池"]
HTTP --> RETRY["指数退避重试"]
LOAD --> HTTP
```
...
- **增强的网络限流**：HostThrottle确保按主机最小间隔限制请求频率，提升稳定性，但会增加端到端延迟；可根据供应商配额调优。进程级会话复用减少连接开销，指数退避重试提高网络鲁棒性。
...
- 外部内容异常：
  ...
  - 确认特殊标记已被中和，避免角色边界伪造。
- **HTTP请求失败**：
  - 检查HostThrottle配置的最小间隔是否合理。
  - 查看指数退避重试日志，确认重试次数和退避时间。
  - 验证User-Agent设置是否符合数据提供商要求。
  - 检查进程级会话池是否正常复用连接。
...
- [agent/backtest/loaders/_http.py:165-196](file://agent/backtest/loaders/_http.py#L165-L196)

## 结论
Vibe-Trading 的安全体系通过多层防护实现了从认证授权、会话管理、沙箱执行、网络控制到实盘风控的全链路安全保障。其核心在于"默认拒绝、最小暴露、强制失败关闭"，配合审计追踪与外部内容扫描，有效降低了提示注入、越权访问、资源滥用与数据泄露等风险。**最新的增强的HTTP限流和连接管理系统进一步提升了金融数据提供商访问的健壮性和稳定性**。建议在生产环境中启用 API 密钥认证、严格 CORS 与 CSP、开启外部内容扫描，并根据业务需求调优沙箱资源限制与网络限流策略。
...
- **增强的网络配置**
  - 配置代理与证书环境变量，确保数据源可达。
  - 设置各数据提供商的最小间隔环境变量（如VIBE_TRADING_EASTMONEY_MIN_INTERVAL、VIBE_TRADING_SINA_MIN_INTERVAL）。
  - 配置SEC User-Agent环境变量（VIBE_TRADING_SEC_UA）以满足合规要求。
...
- [agent/src/live/enforcement.py:455-617](file://agent/src/live/enforcement.py#L455-L617)
- [agent/backtest/loaders/_http.py:113-124](file://agent/backtest/loaders/_http.py#L113-L124)
- [agent/backtest/loaders/sec_edgar_client.py:53-75](file://agent/backtest/loaders/sec_edgar_client.py#L53-L75)
