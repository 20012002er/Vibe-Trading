...
## 更新摘要
**变更内容**
- 新增 DNS 重绑定攻击防护系统，包含自定义中间件类（_HostGuardMiddleware、_OriginGuardMiddleware）
- 实现 Host 头归一化和 Origin 验证机制
- 添加 VIBE_TRADING_MCP_ALLOWED_HOSTS 环境变量详细文档和安全默认行为
- 增强 MCP 传输层的安全防护措施
...
**更新** 新增了全面的 DNS 重绑定攻击防护系统，通过自定义中间件类和严格的主机白名单机制，有效防止恶意网站通过 DNS 重绑定技术访问本地 MCP 服务。
...
F --> G["外部数据源"]
C --> H["安全响应头/CSP/权限策略"]
E --> I["DNS 重绑定防护<br/>主机白名单验证"]
```

**图表来源**
- [agent/src/api/security.py:166-253](file://agent/src/api/security.py#L166-L253)
- [agent/mcp_server.py:229-303](file://agent/mcp_server.py#L229-L303)
...
- **新增** DNS 重绑定与 Host 头校验：对来自回环客户端的请求，严格校验 Host 头是否属于受信任列表，防止通过伪造 Host 绕过本地鉴权。
- **新增** MCP 传输守卫：对 sse/http 传输实施 Host/Origin 白名单校验，默认仅允许回环主机，支持通配符与自定义列表。
...
- [agent/src/api/security.py:166-253](file://agent/src/api/security.py#L166-L253)
- [agent/mcp_server.py:142-303](file://agent/mcp_server.py#L142-L303)
...
### DNS 重绑定与 Host 头验证
- **新增** 回环信任：仅当请求来自回环客户端时，才允许 Host 头匹配受信任列表（默认 localhost/127.0.0.1/[::1] 等）。
- **新增** 拒绝不可信 Host：即使来源为回环，若 Host 不在白名单则直接拒绝，防止 DNS 重绑定绕过。
- **新增** MCP 传输守卫：对 sse/http 传输同样实施 Host/Origin 白名单校验，默认仅允许回环，支持通配符。
- **新增** Host 头归一化：正确处理 IPv6 地址、端口号和大小写，避免标准库处理缺陷。
...
**图表来源**
- [agent/mcp_server.py:229-257](file://agent/mcp_server.py#L229-L257)
...
### Origin 验证与跨域防护
- **新增** Origin 头验证：对浏览器请求的 Origin 头进行白名单校验，防止跨站脚本攻击。
- **新增** 缺失 Origin 处理：非浏览器客户端（curl、Python SDK）不发送 Origin 头，视为可信。
- **新增** 通配符支持：支持 `*` 和 `*.example.com` 形式的域名匹配。
- **新增** 失败关闭策略：当 Origin 头存在但无法解析时，拒绝请求以确保安全。

```mermaid
flowchart TD
OriginReq["收到请求(含Origin)"] --> CheckOrigin{"Origin头存在?"}
CheckOrigin --> |否| AllowNonBrowser["允许非浏览器请求"]
CheckOrigin --> |是| ParseOrigin["解析Origin主机名"]
ParseOrigin --> MatchWhitelist{"匹配白名单?"}
MatchWhitelist --> |是| Allow["允许请求"]
MatchWhitelist --> |否| Deny["拒绝: 不可信来源"]
```

**图表来源**
- [agent/mcp_server.py:208-226](file://agent/mcp_server.py#L208-L226)
- [agent/mcp_server.py:260-286](file://agent/mcp_server.py#L260-L286)
...
- **新增** MCP 传输守卫独立于 API 主流程，针对 sse/http 传输增加 Host/Origin 白名单校验。
...
- **新增** MCP 传输守卫：默认仅允许回环主机，平衡安全性与本地开发便利性。
...
- **新增** 现象：MCP 传输被拒绝
  - 检查 VIBE_TRADING_MCP_ALLOWED_HOSTS 环境变量配置；确认 Host 头格式正确；验证 Origin 头是否匹配白名单。
- 现象：SSE 流无法建立
...
## 结论
Vibe-Trading 在网络层实现了多层防护：严格的 CORS 与 CSP、**新增的** DNS 重绑定防护、Host/Origin 白名单、SSE 票据鉴权、数据加载器边界保护与通道 URL 校验。这些措施共同抵御 DNS 重绑定、跨站请求、XSS、点击劫持与 SSRF 等常见攻击。建议在反向代理层启用 HTTPS 强制与 HSTS，并结合监控与日志脱敏，形成端到端的安全闭环。

**更新** 新增的 DNS 重绑定防护系统通过自定义中间件和严格的主机白名单机制，有效防止了恶意网站通过 DNS 重绑定技术访问本地 MCP 服务的风险，显著提升了本地开发环境的安全性。
...
- **新增** 配置 MCP 传输主机白名单
  - 设置 `VIBE_TRADING_MCP_ALLOWED_HOSTS` 环境变量，指定允许的 Host/Origin 列表。
  - 默认值为空，表示仅允许回环地址（127.0.0.1、::1、localhost），防止 DNS 重绑定攻击。
  - 支持逗号分隔的多个主机名，支持通配符（*、*.example.com）形式。
- 启用 Shell 工具（谨慎）
