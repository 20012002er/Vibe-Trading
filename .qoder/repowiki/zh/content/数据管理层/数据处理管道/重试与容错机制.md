...
## 更新摘要
**所做更改**
- 增强了HTTP层重试机制，支持ConnectionError、Timeout和ProtocolError异常处理
- 添加了指数退避策略和抖动机制
- 改进了市场数据加载器的限流功能
- 更新了会话管理和连接池优化
- 增强了错误日志记录和诊断信息
...
**更新** 增强了HTTP层的综合错误处理能力，现在支持更全面的网络异常类型处理和优化的连接管理。

## 项目结构
围绕重试与容错的关键代码集中在数据加载层与调度执行层：
- 数据加载层（backtest/loaders）：提供统一的"带预算的重试"工具函数，供各数据源 loader 复用。
- HTTP层（_http.py）：提供增强的HTTP请求重试机制，包含指数退避和连接池管理。
...
B --> D["外部API/数据源"]
E["HTTP层(throttled_get)"] --> F["指数退避+连接池管理"]
G["调度器(Scheduled Research)"] --> H["指数退避+最大连续失败"]
I["Swarm 重试(MCP)"] --> J["新建运行(不fork活跃运行)"]
K["配置中心(env_schema)"] --> B
K --> E
K --> G
K --> I
...
- **新增** HTTP层重试：throttled_get 提供增强的HTTP请求重试，支持 ConnectionError、Timeout、ProtocolError 异常处理。

**更新** HTTP层现在支持更全面的异常类型处理和优化的连接池管理。
...
participant HTTP as "throttled_get"
...
Retry->>HTTP : 调用fn() (可能包含HTTP请求)
alt HTTP层重试
HTTP->>HTTP : 指数退避 + 连接池管理
end
alt 返回成功
HTTP-->>Retry : 结果
Retry-->>Caller : 返回结果
...
- [agent/backtest/loaders/_http.py:166-196](file://agent/backtest/loaders/_http.py#L166-L196)
- [agent/backtest/loaders/base.py:243-439](file://agent/backtest/loaders/base.py#L243-L439)
...
### 组件B：HTTP层重试机制（throttled_get）
**新增** 增强的HTTP请求重试机制，提供以下特性：

- **综合异常处理**：支持 ConnectionError、Timeout、ProtocolError 三种网络异常类型
- **指数退避策略**：使用 base_delay * 2^attempt + jitter 的退避算法
- **连接池管理**：进程级会话复用，减少TCP/TLS握手开销
- **主机限流**：按主机键值进行最小间隔控制，防止IP被封禁
- **会话清理**：最后一次重试时关闭旧会话，强制建立新TCP连接
- **抖动机制**：添加随机抖动避免并发请求同步
...
participant Session as "Session池"
participant API as "远程API"
Client->>Throttle : wait(host_key, min_interval)
Throttle-->>Client : 允许发送
Client->>Session : 获取会话
loop max_retries + 1 次
Client->>API : GET请求
alt 成功
API-->>Client : 响应
Client-->>Client : 返回响应
else ConnectionError/Timeout/ProtocolError
Client->>Client : 计算退避时间
Client->>Throttle : 重新等待间隔
alt 最后一次重试
Client->>Session : 关闭旧会话
Client->>Session : 获取新会话
end
Client->>API : 重试请求
end
end
...
- HTTP层依赖：requests库、urllib3异常处理、threading锁机制。
...
HTTP["_http.py<br/>throttled_get"] --> Requests["requests库"]
HTTP --> Urllib3["urllib3异常"]
HTTP --> Threading["threading锁"]
...
**图表来源**
- [agent/backtest/loaders/base.py:127-236](file://agent/backtest/loaders/base.py#L127-L236)
- [agent/backtest/loaders/_http.py:21-33](file://agent/backtest/loaders/_http.py#L21-L33)
...
- **新增** 连接池优化：进程级会话复用减少TCP/TLS握手开销，提升连接效率。
- **新增** 抖动机制：随机抖动避免并发请求同步，提高系统稳定性。
- **新增** 智能会话管理：最后一次重试时主动关闭会话，避免连接池污染。
...
- 现象：网络限流（429）
  ...
  - 参考：[agent/backtest/loaders/base.py:184-236](file://agent/backtest/loaders/base.py#L184-L236)
- **新增** 现象：HTTP连接错误
  - 检查 ConnectionError、Timeout、ProtocolError 的处理逻辑；确认指数退避配置。
  - 参考：[agent/backtest/loaders/_http.py:166-196](file://agent/backtest/loaders/_http.py#L166-L196)
- **新增** 现象：连接池问题
  - 检查会话管理逻辑；确认最后一次重试时的会话清理机制。
  - 参考：[agent/backtest/loaders/_http.py:182-190](file://agent/backtest/loaders/_http.py#L182-L190)
...
## 结论
Vibe-Trading 的重试与容错体系以"预算+退避+超时"为核心，结合"瞬态异常分类"和"本地缓存降级"，在保证稳定性的同时兼顾性能与可观测性。**新增的HTTP层重试机制**提供了更强大的网络异常处理能力，支持多种网络错误类型的自动恢复，并通过指数退避和连接池管理优化了网络请求的性能和可靠性。调度层与 Swarm 层提供了任务级重试与恢复能力，配合配置中心的集中化管理，使系统在不同网络与上游条件下具备弹性与韧性。
...
- **新增** 连接池污染：利用HTTP层的自动会话清理机制；监控连接池使用情况。
- **新增** 并发冲突：利用抖动机制避免同步请求；调整最小请求间隔。
...
- **新增** HTTP层降级：连接失败时自动重试，多次失败后返回错误信息。
...
- **新增** HTTP层测试
  - 模拟 ConnectionError、Timeout、ProtocolError 异常。
  - 验证指数退避时间和抖动效果。
  - 测试连接池管理和会话清理。
- 基准测试建议
