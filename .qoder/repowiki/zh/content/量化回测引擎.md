...
## 更新摘要
**所做更改**
- 增强了HTTP重试机制，特别针对Eastmoney等严格限流政策的数据提供商
- 添加了基于预算的超时控制和指数退避策略
- 改进了网络故障处理和连接池管理
- 更新了数据加载器架构以支持更健壮的网络请求处理
...
**最新更新**：增强了HTTP可靠性，通过健壮的重试机制处理临时网络故障，特别受益于Eastmoney等具有严格限流政策的数据提供商。
...
LBase["loaders/base.py<br/>协议/校验/缓存/重试"]
LReg["loaders/registry.py<br/>数据源注册/降级链"]
LHTTP["_http.py<br/>HTTP节流/重试/会话管理"]
...
- 数据加载器 loaders：统一 DataLoaderProtocol，支持多市场、自动降级、本地缓存、OHLC 校验、**增强的HTTP重试机制**
...
**更新**：数据加载器现在包含增强的HTTP重试机制，包括基于预算的超时控制、指数退避策略和连接池管理，特别适用于Eastmoney等严格限流的数据提供商。
...
## 架构总览
回测主流程由 runner 驱动，按配置选择数据源与引擎，调用 BaseEngine.run_backtest 完成"数据→信号→权重→逐bar执行→指标"的闭环。CompositeEngine 在跨市场场景下维护共享资金池，并将规则委托给各市场专用引擎。**新增的HTTP重试机制确保在网络不稳定或数据提供商限流时仍能可靠获取数据**。
...
participant H as "_http.py<br/>HTTP重试"
...
D->>H : throttled_get() with retry
H->>H : check_budget() + exponential backoff
H-->>D : 成功响应或TimeoutError
...
**更新**：系统现在包含健壮的HTTP重试机制，专门处理临时网络故障和数据提供商限流问题。

#### HTTP重试核心组件
- **retry_with_budget**：带预算限制的重试函数，支持指数退避和超时控制
- **check_budget**：预算检查函数，防止长时间阻塞
- **throttled_get**：带节流和重试的HTTP GET请求
- **HostThrottle**：进程级主机节流器，防止API限流
...
Load --> Throttle["HostThrottle.wait()<br/>最小间隔控制"]
Throttle --> Retry["retry_with_budget()<br/>指数退避重试"]
Retry --> Budget["check_budget()<br/>超时检查"]
Budget --> Validate["validate_ohlc()"]
Validate --> Save["写入parquet缓存"]
Save --> ReturnData["返回DataFrame"]
```

**图表来源**
- [agent/backtest/loaders/base.py:184-236](file://agent/backtest/loaders/base.py#L184-L236)
- [agent/backtest/loaders/_http.py:53-93](file://agent/backtest/loaders/_http.py#L53-L93)
- [agent/backtest/loaders/_http.py:127-196](file://agent/backtest/loaders/_http.py#L127-L196)
...
- [agent/backtest/loaders/base.py:251-439](file://agent/backtest/loaders/base.py#L251-L439)
- [agent/backtest/loaders/_http.py:127-196](file://agent/backtest/loaders/_http.py#L127-L196)
- [agent/backtest/loaders/eastmoney_loader.py:1-177](file://agent/backtest/loaders/eastmoney_loader.py#L1-L177)
...
- **推荐使用 retry_with_budget/check_budget 做网络健壮性**
- 建议在 fetch 后调用 validate_ohlc 保证数据质量
- 若启用本地缓存，可使用 cached_loader_fetch 包装 fetch

**更新**：建议所有网络请求都使用增强的重试机制，特别是对于可能遇到限流的第三方API。
...
- **loaders.base 依赖 _http 提供增强的HTTP重试功能**
...
LoadersBase["loaders/base.py"] --> HTTP["_http.py"]
```
...
- **增强的重试与预算控制**：retry_with_budget 控制最大重试次数与退避时间，结合 check_budget 在分页拉取时快速失败，避免长时间阻塞
- **HTTP节流与连接池**：HostThrottle 确保对同一主机的请求间隔，requests.Session 复用减少TCP/TLS握手开销
- 本地缓存：parquet 内容寻址缓存，避免重复网络请求；读写失败不影响主流程
- 引擎执行：BaseEngine._execute_bars 逐 bar 执行，配合优化器与约束减少无效调仓

**更新**：性能优化现在包括智能重试机制，可以在网络不稳定或API限流时自动恢复，同时通过预算控制防止无限重试。
...
- [agent/backtest/loaders/base.py:251-439](file://agent/backtest/loaders/base.py#L251-L439)
- [agent/backtest/loaders/_http.py:53-93](file://agent/backtest/loaders/_http.py#L53-L93)
- [agent/backtest/loaders/_http.py:104-110](file://agent/backtest/loaders/_http.py#L104-L110)

## 故障排查指南
- 无可用数据源：当所有候选 loader 不可用时，会抛出 NoAvailableSourceError；检查网络与 API Token，或调整 source/fallback 链
- **HTTP重试相关错误**：如果遇到 TimeoutError，检查网络连接和数据提供商限流设置；可通过环境变量调整最小间隔和重试次数
...
**更新**：新增了HTTP重试相关的故障排查指导，包括超时错误处理和限流配置。
...
## 结论
Vibe-Trading 的回测引擎以 BaseEngine 为核心，提供跨市场、可扩展的 bar-by-bar 执行框架；通过 loaders.registry 的灵活降级链与本地缓存保障数据稳定性与性能；**新增的HTTP重试机制显著提高了网络请求的可靠性，特别适用于Eastmoney等严格限流的数据提供商**；factors 系统以面板契约与严格校验确保因子计算的可靠性与可维护性；runner 的安全沙箱隔离策略代码与生产交易环境，降低风险。整体设计兼顾了多市场适配、性能优化与工程稳健性。

**更新总结**：最新的HTTP重试机制通过指数退避、预算控制和连接池管理，大幅提升了数据获取的鲁棒性，特别是在网络不稳定或数据提供商实施严格限流的情况下。
...
- 自定义数据源：实现 DataLoaderProtocol，**推荐使用 retry_with_budget 和 check_budget 增强网络健壮性**，复用重试与缓存工具，确保 OHLC 校验通过
- 与交易系统集成：loader 仅用于历史数据；真实下单与账户操作通过 src.trading/connectors，回测阶段被安全沙箱阻止

**更新**：自定义数据源开发现在强烈建议使用增强的重试机制，特别是对于可能遇到网络问题或限流的第三方API。
